...
- **데이터 우선순위**:
  1. Django API (`https://limdoohwan.pythonanywhere.com/api/market/latest-prices/`) 실시간 데이터 (1순위)
...
### 5. 최근 주요 업데이트 (2026-02-21)
- **보안 강화 및 파일 정리**: GitHub 노출 리스크가 있는 파일(엑셀, 가상환경 등)을 `.gitignore`에 등록하고 Git 추적에서 제외함.
- **코드 내 출처 익명화**: `kakaku` 키워드를 모두 `market`으로 변경하여 데이터 수집 출처 노출을 방지함.
- **프로젝트 구조 슬림화**: 불필요한 분석용 JSON 파일 및 중복 코드를 제거하여 프로젝트 구조를 정돈함.

### 6. 과거 업데이트 내역
- **2026-02-20**: 데이터 소스 단일화, `watches_ui.json` 가격 비우기, `detail.js` UI 정리.
